# Duplicate class 1 (EZ) to augment EZ predict dataset to balance both classes
import os
import numpy as np
...
def train_val_split(X: np.ndarray, Y: np.ndarray, fold: str = "1", num_nodes: int = 827):
    ...
    if fold == "1": # Last 14 patients for validation/First 54 patients for training
        ...
        X_train = np.concatenate((X[:40*num_nodes,:], X[54*num_nodes:68*num_nodes,:]), axis=0)
        
        Y_train = np.concatenate((Y[:40*num_nodes], Y[54*num_nodes:68*num_nodes]), axis=0)

        print('Y_train: %s' % Counter(Y_train))        

        # For the original validation set
        ...
        # For the original training set
        X_train = np.concatenate((X[:26*num_nodes,:], X[40*num_nodes:68*num_nodes,:]), axis=0)
        
        Y_train = np.concatenate((Y[:26*num_nodes], Y[40*num_nodes:68*num_nodes]), axis=0)
        ...
        # For the original training set
        X_train = np.concatenate((X[:12*num_nodes,:], X[26*num_nodes:68*num_nodes,:]), axis=0)
        
        Y_train = np.concatenate((Y[:12*num_nodes], Y[26*num_nodes:68*num_nodes]), axis=0)
        ...
        # For the original training set       
        X_train = X[12*num_nodes:68*num_nodes,:]        
        Y_train = Y[12*num_nodes:68*num_nodes]
...
# function to augment data using duplicate class 1 entries
def augment_data(X: np.ndarray, Y: np.ndarray):
    ...
    # Find the number of duplications required
    num_duplicates = num_class_0 - num_class_1

    # Find the indices of class "1"
    class_1_indices = np.where(Y == 1)[0]

    # Randomly select samples from class "1"
    selected_indices = np.random.choice(class_1_indices, size=num_duplicates, replace=True)

    # Duplicate the selected samples
    X_duplicates = X[selected_indices]
    Y_duplicates = Y[selected_indices]

    # Concatenate the duplicates with the original data
    X_balanced = np.concatenate((X, X_duplicates), axis=0)
    Y_balanced = np.concatenate((Y, Y_duplicates), axis=0)

    print('Y_balanced: %s' % Counter(Y_balanced))
...
def main(root: str, num_nodes: int = 3, fold_no: str = "1"):    
    ...
    # augment the training data using duplicate class 1 samples
    X_train, Y_train = augment_data(X_train_orig, Y_train_orig)  # type:ignore

    # augment the validation data using duplicate class 1 samples
    X_val, Y_val = augment_data(X_val_orig, Y_val_orig)  # type:ignore
        
    # save the augmented data with duolicate class 1 samples as separate nodes
    save_dir_train = 'Train_NonEZvsEZ_whole_brain_duplicate_fold' + fold_no
    if not os.path.exists(save_dir_train):
        os.makedirs(save_dir_train)
    
    save_aug_data_as_separate_nodes(save_dir_train, X_train, Y_train, mode="train")  # type:ignore  

    save_dir_val = 'Val_NonEZvsEZ_whole_brain_duplicate_fold' + fold_no
    if not os.path.exists(save_dir_val):
        os.makedirs(save_dir_val)
...
if __name__ == "__main__":
    ...
    # main(root, num_nodes=827, fold_no="3")
    # main(root, num_nodes=827, fold_no="4")
    # main(root, num_nodes=827, fold_no="5")
