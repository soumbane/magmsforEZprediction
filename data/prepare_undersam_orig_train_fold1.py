...
def undersample_data(X: np.ndarray, Y: np.ndarray, random_state: int = 100):
    ...
    X_under, Y_under = rus.fit_resample(X, Y) # type:ignore
...
def save_aug_data_as_separate_nodes(save_dir: str, X: np.ndarray, Y: np.ndarray, mode: str = "train") -> None:
    ...
    for i in range(len(Y)):
        if mode == "train":
            savemat(os.path.join(save_dir,'X_train_orig_whole_brain_node' + str(i) + '.mat'), {"X_orig_train_node" + str(i):X[i,:]})
            savemat(os.path.join(save_dir,'Y_train_orig_whole_brain_node' + str(i) + '.mat'), {"Y_orig_train_node" + str(i):Y[i]})
...
def main(root: str, num_nodes: int = 3, fold_no: str = "1"):    
    ...
    print('Undersampled Y_train dataset %s' % Counter(Y_train))
