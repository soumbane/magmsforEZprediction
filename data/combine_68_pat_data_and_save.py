...
def load_data(root: str, mode: str = "node_level"):
    ...
    if mode == "node_level":        
        ...
        for j in range(68): # loop over the patients
            for i in node_numbers_with_smote: # loop over all nodes for each patient   
                if j < 44:   
                    ## Load ModelCohort
                    # print(f"Loading ModelCohort for Node num: {i}")
        ...
        raise NotImplementedError(f"Mode must be either node_level or patient_level to load dataset.")
...
def main(root: str, mode: str = "patient_level"):
    ...
    # save the data
    save_dir = 'NonEZvsEZ_whole_brain_' + mode 
