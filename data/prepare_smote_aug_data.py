...
def main(root: str, node_num: str, num_train_aug_samples: int = 100, num_test_aug_samples: int = 100, k_neighbors_train: int = 5, k_neighbors_test: int = 3):
    ...
    X_train = X_combined[:x_train_len,:] # type:ignore
    Y_train = Y_combined[:x_train_len]

    X_test = X_combined[x_train_len:,:] # type:ignore
    Y_test = Y_combined[x_train_len:]
    ...
    savemat('X_test_aug_node' + node_num + '.mat', {"X_aug_test":X_aug_test})
    savemat('Y_test_aug_node' + node_num + '.mat', {"Y_aug_test":Y_aug_test})

    '''# augment all patients data using SMOTE (balance dataset and create more samples)
    X_aug, Y_aug = augment_data(X_combined, Y_combined, k_neighbors = k_neighbors_train, num_samples = num_train_aug_samples, random_state=100)
