...
def augment_data(X: np.ndarray, Y: np.ndarray, num_samples_nonEZ: int = 50, num_samples_EZ: int = 50, random_state: int = 100):
...
def load_model_cohort(root: str, num_samples_nonEZ: int = 50, num_samples_EZ: int = 50, random_state: int = 100):
    ...
    for i in node_numbers_with_smote:    
        ...
        if np.sum(Y_mat_aug_1) == 1:

            ## Load Augmented ModelCohort for Node num: i (Training)
            print(f"Loading Augmented ModelCohort for Node num: {i}")

            path = os.path.join(root,'Train_NonEZvsEZ_ALL')

            RI_file = f"Train_NonEZvsEZ_RI_node{i}_ALL.mat"
            Conn_file = f"Train_NonEZvsEZ_Conn_node{i}_ALL.mat"
            label_file = f"Train_NonEZvsEZ_label_node{i}_ALL.mat"
            RI_mat_name = "Augmented_RI"
            Conn_mat_name = "Augmented_Conn"
            label_mat_name = "Augmented_label"
            ...
            # X_combined_2 and Y_mat_aug_2 are already augmented for node i - so just append them to the bigger lobe matrix
            ...
            # augment each node data using SMOTE when no of EZ > 1
            X_combined_1, Y_mat_aug_1 = augment_data(X_combined_1, Y_mat_aug_1, num_samples_nonEZ=num_samples_nonEZ, num_samples_EZ=num_samples_EZ, random_state=random_state) # type:ignore
...
def main(root: str, save_path_training: str, save_path_validation: str, num_samples_nonEZ: int = 50, num_samples_EZ: int = 50):    
...
if __name__ == "__main__":
    ...
    # save_path_training = '/home/neil/Lab_work/Jeong_Lab_Multi_Modal_MRI/Lobe_Data_exp7/SMOTE_Augmented_Data/'
    # save_path_validation = '/home/neil/Lab_work/Jeong_Lab_Multi_Modal_MRI/Lobe_Data_exp7/Original_Patient_Data/'

    save_path_training = '/media/user1/MyHDataStor41/Soumyanil_EZ_Pred_project/Data/All_Hemispheres/Lobe_Data_exp7/SMOTE_Augmented_Data/'
    save_path_validation = '/media/user1/MyHDataStor41/Soumyanil_EZ_Pred_project/Data/All_Hemispheres/Lobe_Data_exp7/Original_Patient_Data/'
    ...
    main(root, save_path_training, save_path_validation, num_samples_nonEZ=600, num_samples_EZ=500)
