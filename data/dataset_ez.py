...
class DatasetEZ_NodeLevel(Dataset):
    ...
    def __init__(self, batch_size: int, root: str, node_num: int = 1, drop_last: bool = False, mode: EZMode = EZMode.TRAIN, shuffle: bool = False, device=torch.device("cuda:0")) -> None:
        super().__init__(batch_size, drop_last=drop_last, shuffle=shuffle, device=device)
        self.mode = mode
        self.root = root
        ...
        # initialize path
        if self.mode == EZMode.TRAIN:
            self.path = os.path.join(self.root, 'Node_' + str(node_num), 'Train')
            self.x_file = f"X_train_aug_node"
            self.y_file = f"Y_train_aug_node"
            self.x_mat_name = "X_aug_train_node"
            self.y_mat_name = "Y_aug_train_node"
            # self.size = 82

        elif self.mode == EZMode.VALIDATE:
            self.path = os.path.join(self.root, 'Node_' + str(node_num), 'Val')
            self.x_file = f"X_val_aug_node"
            self.y_file = f"Y_val_aug_node"
            self.x_mat_name = "X_aug_valid_node"
            self.y_mat_name = "Y_aug_valid_node"
            # self.size = 22
...
class DatasetEZ_WB_Val_Original(Dataset):
    r"""
    Args:
        root (string): Root directory where the dataset should be saved.
        mode (EZMode): The training/validation/testing mode to load the data
    ...
    def __init__(self, batch_size: int, root: str, drop_last: bool = False, mode: EZMode = EZMode.VALIDATE, fold_no: str = "1", shuffle: bool = False, device=torch.device("cuda:0")) -> None:
        super().__init__(batch_size, drop_last=drop_last, shuffle=shuffle, device=device)
        self.mode = mode
        self.root = root
        ...
        # initialize path
        if self.mode == EZMode.TRAIN:
            raise NotImplementedError("Train data not needed for validation")

        elif self.mode == EZMode.VALIDATE:
            self.path = os.path.join(self.root, 'Val_NonEZvsEZ_whole_brain_original_separate_fold'+fold_no)
            ...
            raise NotImplementedError("Test data not needed for validation")
...
class DatasetEZ_WB_ALL_Original(Dataset):
    r"""
    Args:
        root (string): Root directory where the dataset should be saved.
        mode (EZMode): The training/validation/testing mode to load the data
...
class DatasetEZ_WB_SubGroupAnalysis(Dataset):
    r"""
    Args:
        root (string): Root directory where the dataset should be saved.
        mode (EZMode): The training/validation/testing mode to load the data
    ...
    def __init__(self, batch_size: int, root: str, drop_last: bool = False, mode: EZMode = EZMode.VALIDATE, fold_no: str = "1", shuffle: bool = False, device=torch.device("cuda:0")) -> None:
        ...
        if self.mode == EZMode.TRAIN:
            ...
            self.path = os.path.join(self.root, 'Val_NonEZvsEZ_WB_SubGroup_Analysis','Fold1','SF0')
            self.x_file = f"X_val_aug_whole_brain_node"
            self.y_file = f"Y_val_aug_whole_brain_node"
            self.x_mat_name = "X_aug_valid_node"
            self.y_mat_name = "Y_aug_valid_node"
...
if __name__ == "__main__":
    ...
    print("Whole Brain Original EZ Dataset ...")
    ez_dataset = DatasetEZ_WB_ALL_Original(batch_size=1, root='/home/neil/Lab_work/Jeong_Lab_Multi_Modal_MRI/magmsforEZprediction/', drop_last=False, mode=EZMode.VALIDATE, fold_no="1", shuffle=False)
