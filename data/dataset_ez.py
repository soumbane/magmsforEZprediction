...
class DatasetEZ_WB(Dataset):
    ...
    def __init__(self, batch_size: int, root: str, drop_last: bool = False, mode: EZMode = EZMode.TRAIN, shuffle: bool = False, device=torch.device("cuda:0")) -> None:
        ...
        # initialize path
        if self.mode == EZMode.TRAIN:
            self.path = os.path.join(self.root, 'Train_NonEZvsEZ_whole_brain_aug_separate')
            ...
            self.path = os.path.join(self.root, 'Val_NonEZvsEZ_whole_brain_aug_separate')
        ...
        # self.data = X_multi_modal, Y_label
...
if __name__ == "__main__":
    ...
    ez_dataset = DatasetEZ_WB(batch_size=1, root='/home/neil/Lab_work/Jeong_Lab_Multi_Modal_MRI/baselines_all_hemispheres/', drop_last=False, mode=EZMode.TEST, shuffle=True)
