...
SCRIPT_NAME="relational_reasoning_all_nodes_all_trials_left.py"  # Change this to your actual Python script name

# Run the Python script with nohup
nohup python $SCRIPT_NAME > logs_left/output.log 2>&1 &

# Save the process ID to a file for later reference
echo $! > logs_left/process_pid.txt

echo "Process started with PID $(cat logs_left/process_pid.txt)"
echo "Output is being logged to logs_left/output.log"