...
# Log file
LOG_FILE="logs_add_eval_right4/evaluation_progress.log"

# Create a temporary script file for the loop
cat > temp_eval_script_add_right4.sh << 'EOF'
#!/bin/bash
LOG_FILE="logs_add_eval_right4/evaluation_progress.log"
...
do
    ...
    python eval_right.py /media/user1/MyHDataStor41/Soumyanil_EZ_Pred_project/Data/All_Hemispheres/Right_Hemis/Part_2/ /media/user1/MyHDataStor41/Soumyanil_EZ_Pred_project/Models/magmsforEZprediction/experiments/ -b 4 --node_num ${node_num} -exp eval_add_right4.exp --replace_experiment --show_verbose --device cuda:0 >> $LOG_FILE 2>&1
    
    # Record the experiment completion
    echo "[$(date)] Done Evaluating ALL modalities for node_num ${node_num}" >> $LOG_FILE
    
    # Save progress information
    echo "$(date) - Completed: ${node_num}" >> logs_add_eval_right4/completed_nodes.txt
done
...
chmod +x temp_eval_script_add_right4.sh

# Run the temp script with nohup, passing the array elements as arguments
nohup ./temp_eval_script_add_right4.sh "${node_nums[@]}" > /dev/null 2>&1 &

# Save the process ID
PID=$!
echo $PID > logs_add_eval_right4/evaluation_pid.txt
