...
# Log file
LOG_FILE="logs_bonn_eval_right0/evaluation_progress.log"

# Create a temporary script file for the loop
cat > temp_eval_script_bonn_right0.sh << 'EOF'
#!/bin/bash
LOG_FILE="logs_bonn_eval_right0/evaluation_progress.log"
...
chmod +x temp_eval_script_bonn_right0.sh

# Run the temp script with nohup, passing the array elements as arguments
nohup ./temp_eval_script_bonn_right0.sh "${node_nums[@]}" > /dev/null 2>&1 &

# Save the process ID
PID=$!
echo $PID > logs_bonn_eval_right0/evaluation_pid.txt
