...
# Log file
LOG_FILE="logs_add_left2/training_progress.log"

# Create a temporary script file for the loop
cat > temp_training_script_add_left2.sh << 'EOF'
#!/bin/bash
LOG_FILE="logs_add_left2/training_progress.log"
...
chmod +x temp_training_script_add_left2.sh
...
nohup ./temp_training_script_add_left2.sh "${node_nums[@]}" > /dev/null 2>&1 &

# Save the process ID
PID=$!
echo $PID > logs_add_left2/training_pid.txt
