...
mkdir -p logs_add_right5

# Log file
LOG_FILE="logs_add_right5/training_progress.log"

# Create a temporary script file for the loop
cat > temp_training_script_add_right5.sh << 'EOF'
#!/bin/bash
LOG_FILE="logs_add_right5/training_progress.log"
...
do
    ...
    echo "$(date) - Completed: ${node_num}" >> logs_add_right5/completed_nodes.txt
done
...
chmod +x temp_training_script_add_right5.sh
...
nohup ./temp_training_script_add_right5.sh "${node_nums[@]}" > /dev/null 2>&1 &

# Save the process ID
PID=$!
echo $PID > logs_add_right5/training_pid.txt
