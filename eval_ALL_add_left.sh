...
# Evaluate every left hemisphere node on the additional cohort:
...
# Each of the 3 training trials is written as its own column.
#
# Runs all 286 nodes sequentially. For a parallel run, use the shards in eval_sh_scripts/nh_eval_add_left*.sh
